    ...
    return (
        <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 text-center">
                <h2 className="text-gray-500 uppercase tracking-wide text-xs font-semibold mb-1">
                    Today's Goal
                </h2>
                <div className="text-4xl font-extrabold text-gray-900 mb-2">
                    {Math.round(totalPoints * 10) / 10} <span className="text-gray-400 text-2xl">/ {targetPoints}</span>
                </div>
                ...
                <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${isMet ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
            ...
            {entries.length > 0 ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                    <p className="text-gray-500">No push-ups logged yet today.</p>
                    <p className="text-sm text-gray-400 mt-1">Get down and give me {targetPoints}!</p>
                </div>
            )}
